...
# taking equally spaced 1000 points between 0 and 1 for graph plotting
pdf = (np.linspace(0.0, 1.0, num=1000))
pdf = np.sort(pdf, axis = None)
# print(pdf)

# calculating maximum likelihood (not between 0.4 and 0.6)
uml = np.random.random()
while (uml >= 0.4 and uml <= 0.6):
    uml = np.random.random()

#generating datapoints based on uml
